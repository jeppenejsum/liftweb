...
trait CouchRecord[MyType <: CouchRecord[MyType]] extends JSONRecord[MyType] {
    ...
    object id extends JSONOptionalStringField(this, 100) { override def jsonName = Some("_id") }

    /** The mandatory _rev field */
    object rev extends JSONOptionalStringField(this, 100) { override def jsonName = Some("_rev") }
...
trait CouchMetaRecord[BaseRecord <: CouchRecord[BaseRecord]] extends JSONMetaRecord[BaseRecord] {
    ...
    def save(inst: BaseRecord): Box[Unit] = {
        ...
        try {
            for (newDoc <- http(inst.database store inst.asJValue)) yield {
                inst.id.set(newDoc._id)
                inst.rev.set(newDoc._rev)
                ()
            }
        } finally {
    ...
    def saved_? (inst: BaseRecord): Boolean = inst.id.value.isDefined && inst.rev.value.isDefined
